...
export function CardShogiGame({
  ...
  // 仕様 (2026-07-06): AI が二手指しを確定 (CONFIRM → doubleMove state 発生、movesLeft===2) した
  // 瞬間に、使用する **二手指しカードを手札で表向き + 緑枠 + 発光** させ「このカードを使うぞ」を予告
  // + 確定音 (card_use_confirm = 人間が「使用する」を押したときの音)。1手目は hook 側が
  // DOUBLE_MOVE_INTRO_MS 待って dispatch するため、予告を見せ切ってから駒が動く。カード本体の
  // 中央使用アニメ (CardPlayFlight + 刀の素振り) は 2手目完了時の自然フロー (activateStep cardUse)
  // で発火 = 「2手目と同時にカード使用アニメ」。強調はカードが手札から消費される 2手目完了で
  // doubleMove===null になり自然解除する。
  // 人間の二手指し (active===playerColor かつ非観戦) は自分で確定操作するため対象外。
  // movesLeft 2→1 遷移や待った (1→2 復帰) では fireKey ガードで再発火しない。
  const [aiDoubleMoveHighlightId, setAiDoubleMoveHighlightId] = useState<string | null>(null);
  const aiDoubleMoveIntroFiredRef = useRef<string | null>(null);
  useEffect(() => {
    if (!doubleMove) {
      aiDoubleMoveIntroFiredRef.current = null;
      setAiDoubleMoveHighlightId(null); // カード消費 (2手目完了) で強調解除。
      return;
    }
    const isAiSide = spectatorMode || doubleMove.active !== playerColor;
    if (!isAiSide) return;
    if (doubleMove.movesLeft !== 2) return;
    ...
    setAiDoubleMoveHighlightId(doubleMove.cardInstance.instanceId);
          ...
          if (ev.kind === "cardPlayEvent") {
            ...
            if (ev.player === playerColor) {
              ...
              if (!opponentPieceFlightFired) {
                // 二手指しを含め、相手カード使用の中央アニメ (CardPlayFlight) はここ (2手目完了時の
                // cardPlayEvent) で発火 = 「2手目と同時にカード使用アニメ」。音は通常カードと共通の
                // card_use_animation (刀の素振り)。冒頭の予告 (手札ハイライト+確定音) は別 effect。
                playFlightKeyRef.current += 1;
                playSfx("card_use_animation");
  ...
  const opponentHandFaceDown = (
    <div data-hand-area>
      <HandArea
        ...
        size="sm"
        emptyLabel=""
        highlightRevealCardId={aiDoubleMoveHighlightId}
      />
    </div>
  );
