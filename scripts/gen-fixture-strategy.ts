...
function main() {
  ...
  const spectatorMeta = {
    ...
    // ZZ-A 解消 (PR1d-1 実装第 2 次レビュー ZZ-A、PR1d-4 で同時解消):
    // 旧 pliesPerScenario:50 は「上限値」と「実態 ply 数」が混同される問題が
    // あった (実態は evaluateGameEnd break で 25 ply 等、4 シナリオで異なる)。
    // maxPliesPerScenario (上限) と actualPliesPerScenario (各シナリオ実態
    // moves.length) に分離して構造的に解消。
    maxPliesPerScenario: SPECTATOR_PLIES_PER_SCENARIO,
    actualPliesPerScenario: spectatorScenarios.map((s) => s.moves.length),
