...
### PR1d-4 実装 設計判断反映 (2 件ユーザー確認済 + 実装ギャップ ZZ)

PR1d-4 実装中、計画書 PR1d-4 詳細 (L1261-1519) の擬似コードと実装の整合性が問題となり、ユーザーに 2 件の設計判断を確認:

| # | 設計判断 | ユーザー確定 | 反映内容 |
|---|---------|------------|---------|
| **ギャップ 1** | no_promote マーク評価 (計画書 L1310 `noPromoteMarksPositions` 配列 + L1368 `evaluateNoPromoteMarksProximity` 玉位置依存) | **案 A (単純カウント差)** | `evaluateCardDigest` は `(digest, variant)` のみで GameState/玉位置を持たない (W-1 root スカラー方式、GameState 非依存が設計の核) ため proximity 評価は実装不可。CardDigest フィールドを `noPromoteMarksPositions: {row,col,player}[]` から `noPromoteMarkCountDelta: number` (sente 数 - gote 数、玉位置非依存) に簡略化。`NO_PROMOTE_MARK_COEFFICIENT` (計画書 L1395 `NO_PROMOTE_PROXIMITY_BONUS=30` を proximity でなくカウント係数に流用) |
| **ギャップ 2** | perf-bench の PR1c-2 baseline 比較 (計画書 L1430-1431) | **案 A (測定+ログのみ)** | PR1c-2 完了時点 baseline 値が未測定のため、baseline 閾値比較はコメントアウト維持。`depthCompleted` / `nodes` / `elapsedMs` を測定・ログ出力のみ。将来 baseline 確立時に閾値比較を有効化 |

**実装ギャップ (擬似コードと実コードの差異、ZZ 記録)**:

- action-generator.ts は no_promote/check_break 候補生成のため変更不要 (`kind:"trap"` + `targeting:"none"` を PR1d-2 の `getCardActions` 項目5 (`hasSameKindTrapPlaced` 同種トラップ重複) + 項目7 (`checkUsage:"forbidden"` 王手中除外) + `enumerateTargets` case "none" が既に対応。計画書 L1288「候補追加」は実態と差異)
- evaluateAction の no_promote/check_break は固定価値近似 (現局面評価 + `TRAP_VALUE_*`、draw の `DRAW_VALUE_BONUS` と同型)。計画書 L1366-1368 の proximity 評価は GameState 非依存のため不可、bench で `TRAP_VALUE_*` 係数調整 (計画書 L1267 トラップ系リスク警告に対応)
- perf-bench は `RUN_PERF_BENCH=true` 明示時のみ実行 (計画書 L1408 `SKIP_IN_CI=CI||SKIP_PERF_BENCH` はローカル test:ci でも実行されてしまい G-4「通常テストで bench を走らせない」に反するため、`describe.skipIf(!RUN_PERF_BENCH)` 方式に変更)
- perf-bench 局面は初期局面+数手進めた代表局面 (案 A で baseline 比較なしのため厳密 50 局面 fixture 不要、計画書 L1400「50 局面」と差異。将来 baseline 確立時に整備)
- **ZZ-A 解消 = B案 (meta のみ手動更新、fixture 再生成せず)**: strategy/spectator fixture とも `gen-fixture-strategy.ts` L203/L249 で cardState 未渡のため PR1d-1〜4 の cardDigest 影響なし。現 spectator-baseline.json は 4 シナリオ全 25 ply で確定 (計画書 L1759 想定と一致)、再生成しても内容同一 (= 50 分の無駄)。よって gen-fixture-strategy.ts を `maxPliesPerScenario` + `actualPliesPerScenario` 出力に構造変更 (次回再生成時の正値保証) + spectator-baseline.meta.json を `maxPliesPerScenario:50` + `actualPliesPerScenario:[25,25,25,25]` に手動更新。計画書 C-3「PR1d-4 で観戦モード新基準再生成」は cardState 未渡で振る舞い不変のため「meta 構造のみ ZZ-A 解消」に縮約 (ZZ 反映)

PR1d-4 はコミット 1 (CardDigest 拡張、`37b32b4`) + コミット 2 (evaluateCardDigest トラップ価値、`5d8c20e`) + コミット 3 (evaluateAction トラップ評価経路、`3201431`) + コミット 4 (perf-bench 整備、`1f6d1ee`) + コミット 5 (ZZ-A meta 構造解消) の 5 コミットで完結。card-digest +6 / evaluate-action +3 / action-generator +3 / perf-bench 2 ファイル新規、test:ci 397 passed + perf-bench 6 skipped (回帰なし)。
