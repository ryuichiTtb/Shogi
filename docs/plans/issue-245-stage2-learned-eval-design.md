...
> M1 レビュアー推奨値は §12 を参照。**ユーザー決定 (2026-06-28、いずれも推奨採用)**: 探索深さ **D=4 起点** (P2-0 で生成時間を実測し 4↔5 微調整) / 混合 **α≈0.5 起点** (cp 空間で混合してから squash、対戦勝率で ±0.2 校正) / ネット規模 **hidden=32 据置先行** (ラベル濃化の効果を単独評価、頭打ち後に拡大)。混合 α は **§3.4 の cp 空間で混合してから squash** する点に注意 (素朴な加算ではない)。
