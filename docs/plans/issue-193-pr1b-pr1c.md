...
| 新規 | `src/lib/shogi/ai/__tests__/legal-moves.test.ts` | fixture を import して `getSearchLegalMoves` の出力 set が `getFullLegalMoves` と完全一致を検証 |
...
- 200-300 局面を以下のパターンで網羅 (親計画 md L270-274 準拠):
  1. **駒種 8 種 × 成り駒 6 種 × 成りゾーン進入境界** (~40 局面、template ベース)
  2. **王手中の合駒・回避** (打ち歩詰め禁則含む、~40 局面、template ベース)
  3. **ピン駒** (飛角香の利き上の自駒、~30 局面、template ベース)
  4. **持ち駒の打ちたて** (二歩禁、行きどころなし、打ち歩詰め、~50 局面、template ベース)
  5. **王手放置の自殺手禁止** (~30 局面、template ベース)
  6. **平凡な中盤・終盤局面** (random walk、~50 局面)
...
- **template ベース** (パターン 1-5): 駒配置・王手位置等を template として手書き定数で持ち、各 template から数バリエーション (パラメータ変動) を Mulberry32 で生成
- **random walk** (パターン 6-7): 初期局面から合法手を Mulberry32 seed で 1 手選びながら walk、`state.status === "active"` の局面のみ採用 (PR1c の random 生成と同じ手法)
...
## 第 2 次レビュー指摘の反映履歴 ([#issuecomment-4415540843](https://github.com/ryuichiTtb/Shogi/issues/193#issuecomment-4415540843))
