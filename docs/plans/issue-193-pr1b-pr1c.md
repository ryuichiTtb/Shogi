...
| 編集 | `src/lib/shogi/ai/search.ts` | 3 箇所の `getFullLegalMoves` 呼出を `getSearchLegalMoves` に置換 |
...
- ✅ **G-4 (CI 分離)**: `npm run gen:fixture:evaluate` は手動実行を前提、CI で fixture 再生成は走らせない (生成済 JSON を git 管理して CI は読込のみ)
...
- 親計画 md (正本): [docs/plans/issue-193.md](issue-193.md) (740 行)
  - PR1b 詳細: L259-281
  - PR1c 詳細: L283-305
- 引き渡しガイド: [Issue #193 #issuecomment-4415459081](https://github.com/ryuichiTtb/Shogi/issues/193#issuecomment-4415459081)
...
3. **fixture 生成戦略**: PR1b hardcode / PR1c script 化の使い分けは適切か、`scripts/gen-fixture-evaluate.ts` の設計 (乱数 seed 管理 / 局面ミックス) は妥当か
4. **ブランチ運用**: PR1b と PR1c を同セッションで両方完成させる方針 + `origin/main` 直派生は AGENTS.md と整合するか
5. **進行中チェックリスト 19 件との整合**: 本フェーズで対応すべき項目 (G-4 / D-5) の判断は妥当か、見落としはないか
