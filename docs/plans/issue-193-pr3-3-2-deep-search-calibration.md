...
### 1.3 ユーザー決定事項 (2026-06-01、スコープ確定)

ユーザーレビューで以下を確定(詳細 §11):

- **D-1 全難易度でカードを適切に使う**: 初級・中級・上級・超上級の **4 段階すべて**でカード/ドローが適切に使われることを要件とする。「カード将棋なのだから当然」。**中級 (intermediate) は PR3-3 ベンチで未測定だった抜けがあり、本 PR で是正**。「適切に」は難易度ごとに段階化(初級は弱さ演出を維持しつつカードも使う / 超上級は最善運用)。
- **D-2 2 段階スコープ**:
  - **本 PR (PR3-3-2)** = 全難易度の「自分のカード深読み」+ 中級ベンチ是正。相手応答は 1-ply scan 維持(自分側の深掘り)。
  - **次 PR (PR3-4)** = 超上級の「相手のカードも含めた読み」。
- **D-3 超上級の相手カード読み = 期待値モデル(フェア)**: 相手の手札は**覗かない**。「相手はこういうカードを持っていそう」と確率推定して読む(不完全情報モデル)。全知(手札覗き見)は採らない。→ PR3-4 スコープ。

本計画は (C) 深さ N 波及の MVP(自分側深読み)を、上記 D-1〜D-3 に沿って全難易度対応に拡張したもの。
...
深読み budget を **難易度別パラメータ化**する(`DIFFICULTY_PARAMS` に `cardSearchBudget` フィールド追加、単一定数より自然・調整容易・マジックナンバー回避)。budget=0 のとき `evaluateActionDeep` は既存 1-ply lookahead と同一(後方互換)なので、低難易度は校正のみで成立する:

| 難易度 | 探索深さ/時間 | 自分カード深読み budget | 相手カード考慮 | 「適切」の意味 |
|---|---|---|---|---|
| 初級 (beginner) | depth3 / 800ms / ノイズ50% | 0(校正のみ。1-ply で十分) | なし | カードも使うが最善でない=人間が勝てる弱さを維持 |
| 中級 (intermediate) | depth6 / 1800ms / ノイズ10% | 0〜1(bench で決定) | なし | 中盤でも自然にカードを使う |
| 上級 (advanced) | depth16 / 3000ms | 1〜2 | なし(相手は駒のみ) | 最善のカード運用 |
| 超上級 (expert) | depth24 / 3500ms | 2 | **PR3-4 であり**(期待値モデル) | 相手のカードまで見越した最善 |

- **本 PR (PR3-3-2)** は上表の「自分カード深読み budget」列(全難易度)+ 中級ベンチ是正までを実装。budget 値は bench で校正(初期値は上表)。
- **超上級の「相手カード考慮」列は PR3-4**(D-3 = 期待値モデル、§8)。
- 低難易度で budget を小さく保つことで、初級の弱さ演出(ノイズ・浅さ)を壊さず、explosion も発生しない。
...
- [ ] **全 4 難易度(初級/中級/上級/超上級)**で実プレイ相当 midgame fixture のカード/ドロー使用率が「難易度相応に適切」(§3.7 ティア基準。中級ベンチを新規追加。数値目標は K 確定後に設定)
- [ ] 初級の弱さ演出(ノイズ・浅さ・人間が勝てる)が壊れていない(過剰に強くなっていない)
...
- **PR3-4 (相手期待値モデル) = 超上級の相手カード考慮 (D-3 確定)**: 本 PR の相手応答は決定的 1-ply・move-only。PR3-4 で **超上級 (expert) のみ**、相手の手札を**覗かず**「相手はこういうカードを持っていそう」と確率推定する**期待値モデル(フェア)**で、相手のカード/ドロー応答も読みに含める。全知(手札覗き見)は不採用。実装時に deep 相手読みを入れる場合は TT 分離(cardDigest hash キー)が必要(§3.3)。
...
**着手前のユーザー判断(§1.3、解決済):**
- **P-A (代替レバー) → 解決**: 「lookaheadPly を増やすだけ」案は採らず、深読み budget で実装に進む(2 段階スコープ D-2)。budget=0 が実質 lookaheadPly 据え置きに相当するため、低難易度では結果的に安価な経路を内包する。
- **P-B (PR3-4 先行) → 解決**: 2 段階で進める(D-2)。本 PR(自分カード深読み)を先、PR3-4(超上級の相手カード = 期待値モデル)を後。
- **P-C (MVP の深さ)**: K=1 で実プレイ改善が観測できるかは **bench で実測**(§5.3)。未達なら §3.6 の絞り込み / K 調整。難易度別 budget(§3.7)で各レベル最適化。
...
## 11. ユーザーレビュー反映 (スコープ確定、2026-06-01)

§10 の adversarial verify 反映後、ユーザーレビューで以下を確定(§1.3 D-1〜D-3 / §3.7 / §6 / §8 に反映済):

| # | ユーザー要望 | 反映 |
|---|---|---|
| D-1 | **初級・中級でもカードを適切に使ってほしい**(カード将棋なのだから) | DoD を **全 4 難易度**に拡張(§6)。**中級は PR3-3 ベンチ未測定だった抜けを是正**。難易度ティア(§3.7)で「適切」を段階定義(初級は弱さ維持しつつ使用 / 超上級は最善)。深読み budget を `DIFFICULTY_PARAMS.cardSearchBudget` で難易度別パラメータ化 |
| D-2 | **2 段階スコープ** | 本 PR (PR3-3-2) = 全難易度の自分カード深読み + 中級是正。次 PR (PR3-4) = 超上級の相手カード考慮。§8 に明記 |
| D-3 | **超上級は相手(プレイヤー)のカードも含めて読む。ただしフェアに** | PR3-4 で **期待値モデル(相手手札は覗かず確率推定)**。全知(手札覗き見)は不採用。§8 に確定記録 |

> D-1 により、初版の「advanced/expert の midgame カード 0 を解消」という狭い目標から、**全難易度でカードが難易度相応に使われる**へ目標を拡張。低難易度は budget=0(校正のみ)で弱さ演出を保ちつつカード使用を担保する設計とした。
