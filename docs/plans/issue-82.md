...
## 🔁 セッション引継ぎメモ (2026-05-03 — Vol.3 進行中)

### 本セッション (Vol.3) の進捗
- **double_pawn 再調整**: cost 3→2, rarity rare→common (commit `9fcb512`)。通常カード枠へ移動。
- **6 枚目カード `check_break` (王手崩し) 実装**:
  - kind: `trap`, cost 5, rarity `rare`, icon ⚔️
  - 発動: 自玉が王手された瞬間に自動発動
  - 効果: 王手駒すべてを取り、自分の持ち駒に unpromote 加算 (両王手対応)
  - 反復除去でディスカバードチェックも完全解消
  - 演出: 王手中央表示 (1600ms) → トラップ発動演出 (2300ms) → 駒フライト並行
  - 演出中は AI 思考とユーザー操作をロック (`isCheckBreakAnimating`)
- 検証用ハック `TEST_CARD_IDS` は #120 で既に削除済 → DB seed 経由で検証可能 (`status: "active"` なので自動投入)

### 残タスク
- Vercel preview で動作確認 (王手中央表示 → トラップ発動演出 → 駒フライトの順序、両王手・成駒・no_promote 相互作用)
- ユーザー判定 → リリース確定
...
| 6 | `check_break` | 王手崩し | trap | rare | 5 | **実装中 (Vol.3)** — 2026-05-03、相手が王手してきた瞬間に自動発動するトラップ。王手をかけている駒(両王手・複数王手なら全駒)を持ち駒化。成駒は unpromote。反復除去でディスカバードチェックも解消。演出順: 王手中央表示 → トラップ発動演出 → 駒フライト並行 |
