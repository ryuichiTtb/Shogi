...
#### 今回採点する教材の構成 (2026-07-27 決定)

事前監査 (5 視点 × 反証、75 エージェント) の結果を踏まえた構成:

| 入れる | 内容 | 理由 |
|---|---|---|
| ✅ 新教材 350 局 | 49,410 局面 (clean 後) | 多様性ゲート全達成。本命 |
| ✅ 分岐 105 本 | 中終盤を厚くする | 段5 の狙い。局数が少なく費用も小さい |
| ✅ 人間 1 局 | 53 局面 | 唯一の実戦データ。極小 |
| ⏸ 旧教材 348 局 | 26,012 局面 | **今回は見送り** |

**旧教材を見送る理由**: (1) 採点に +12 時間かかり、予算 (1〜2 日) の上限を超える。
(2) 旧教材は bolt-on エンジン + カード下駄で生成されており、カード使用率 14.3% (新教材は 6.0%)。
「必要でもないのにカードを使う」という、まさに直そうとしている振る舞いの見本になっている。
(3) レシピが同じなので**後から追加で採点して結合できる** (取り合い方式は追記式)。
量が足りなければ次ラウンドで足す。

**分岐を入れる判断** (監査は見送りを推奨したが採用しない): 監査の理由は「同じ chooser を使うから
悪手が混じる」だが、それは本教材 350 局にも等しく当てはまり、かつ**ラベルは指し手の質と独立**に
付く。中終盤の網羅は段5 の主目的で、費用も 2 時間と小さい。
ただし `CLEAN_IN` では**末尾に**並べる (親より後に置く)。
