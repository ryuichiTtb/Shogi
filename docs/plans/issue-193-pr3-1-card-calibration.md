...
| ③ | 手札価値が指数飽和 | [src/lib/shogi/ai/cards/digest.ts:87-89](src/lib/shogi/ai/cards/digest.ts#L87) `computeHandValue` (`HAND_VALUE_BASE=20`, `HAND_VALUE_DECAY=3.0`) | `20 × (1 - exp(-handSize / 3))` は 3 枚で 12.64 cp (63%)、5 枚で 16.22 cp (81%)、7 枚で 18.06 cp (90%)。限界価値 3→4 枚 ≒ +2.09 cp / 4→5 枚 ≒ +1.49 cp / 5→6 枚 ≒ +1.07 cp なので **3-5 枚帯の限界価値は十分残る** (= 当初の「3-4 枚で価値ほぼゼロ」narrative は実値ベースでは事実誤認)。問題は絶対値が早期飽和して **5 枚以降の差が小さくなる**こと (5 枚 81%、7 枚 90%、10 枚 96%)。(PR3-3 C-9/C-14 / レビュー F-3 で実値・narrative 訂正、旧記述「19.0 cp / 95% / 限界 0.86」は分母 DECAY を取り違えた誤計算) |
...
### 4.3 `handValue` 減衰再校正 (PR3-3 C-9/C-14 で再検証・narrative 訂正)

現状 `HAND_VALUE_DECAY = 3.0`:
- 絶対値: 3 枚で 12.64 cp (63%)、5 枚で 16.22 cp (81%)、7 枚で 18.06 cp (90%)、10 枚で 19.29 cp (96%)
- 限界価値: 3→4 = +2.09 cp、4→5 = +1.49 cp、5→6 = +1.07 cp (= **3-5 枚帯の追加 1 枚価値は 1 cp 以上残っており**、当初の「3-4 枚で価値ほぼゼロ」は事実誤認 = レビュー F-3)

校正案: **`HAND_VALUE_DECAY = 5.0`** に変更 (本 PR で採用)

| 手札枚数 | DECAY=5.0 (新)   | DECAY=3.0 (旧)    | DECAY=5.0 限界 (n-1→n) | DECAY=3.0 限界 (n-1→n) |
|---|---|---|---|---|
| 1 枚 |  3.63 cp (18%)   |  5.67 cp (28%)    | 0→1 = +3.63 cp       | 0→1 = +5.67 cp       |
| 3 枚 |  9.02 cp (45%)   | 12.64 cp (63%)    | 2→3 = +2.43 cp       | 2→3 = +2.91 cp       |
| 4 枚 | 11.01 cp (55%)   | 14.73 cp (74%)    | 3→4 = +1.99 cp       | 3→4 = +2.09 cp       |
| 5 枚 | 12.64 cp (63%)   | 16.22 cp (81%)    | 4→5 = +1.63 cp       | 4→5 = +1.49 cp       |
| 7 枚 | 15.07 cp (75%)   | 18.06 cp (90%)    | 6→7 = +1.09 cp       | 6→7 = +0.77 cp       |
| 10 枚 | 17.29 cp (86%)  | 19.29 cp (96%)    | 9→10 = +0.60 cp      | 9→10 = +0.28 cp      |

**実際の変更効果** (旧 narrative が誤計算ベースだったため再整理):
- 絶対値: 全体的に低下 (DECAY=5.0 は同じ手札数でも `handValue` が低い、5 枚で 63% vs 旧 81%)
- 限界価値: **3-5 枚帯では DECAY=5.0 と 3.0 でほぼ同等** (~1.5〜2.0 cp)、しかし 7+ 枚帯では DECAY=5.0 の方が高い (n=7: +1.09 vs +0.77)
- 飽和点が後ろにずれる: DECAY=3.0 は n=5 で 81% 飽和 / DECAY=5.0 は n=5 で 63%、**5+ 枚帯でも handValueDelta に差分余地が残る**

**校正の真の意義** (= PR3-3 C-6 wiring 後の context で再評価):
- digest の `handValueDelta` (= sente.handValue - gote.handValue) が action 適用前後で動く範囲が広がる (DECAY=5.0 の方が n=5+ で差分余地が残るため)
- これにより updateCardDigest 経由で hand 変化が action score に効きやすくなる
- bench で `3.0 / 5.0 / 6.0` を比較し最適点を採用 (本 PR では 5.0 採用、bench で sanity)
