...
| ③ | 手札価値が指数飽和 | [src/lib/shogi/ai/cards/digest.ts:87-89](src/lib/shogi/ai/cards/digest.ts#L87) `computeHandValue` (`HAND_VALUE_BASE=20`, `HAND_VALUE_DECAY=3.0`) | `20 × (1 - exp(-handSize/3))` は 3 枚で 19.0(95%)、5 枚で 19.8 — 4 枚目以降の追加価値は 1 cp 未満。中盤に手札がすでに 3-4 枚あればドロー価値ほぼゼロ |
...
現状 `HAND_VALUE_DECAY = 3.0` は 3 枚で 95% 飽和。中盤に手札 3-4 枚を想定すると追加 1 枚価値が 1 cp 未満。

校正案: **`HAND_VALUE_DECAY = 5.0`** に変更
- 3 枚: 91% (18.2 cp)
- 5 枚: 99% (19.7 cp)
- 4 枚目までの差は +1.5 cp 程度 (5 倍延伸)
- bench で `4.0 / 5.0 / 6.0` を比較し最適点を採用
...
| C-6 | chore: `strategy-baseline` 再生成 | `npm run gen:fixture:strategy --variant=card-shogi --skip-opening` 実行結果をコミット | 既存 strategy test green |
| C-7 | docs: メモ更新 | `project-issue-193-roadmap.md` を PR3-1 完了状態に更新(後続 PR3-2/3/4 の予定明記) | — |
