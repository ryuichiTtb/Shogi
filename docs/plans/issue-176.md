...
- Vercel側の `maxDuration` 上限は契約プランに依存する (Hobby 10秒 / Pro 60秒等)。`5` を採用する場合も実装着手時に最新仕様と本リポジトリの契約プランを確認してから確定する
- 現行 `timeLimitMs` は `src/lib/shogi/ai/engine.ts` で beginner 1000 / intermediate 2000 / advanced 4000 / expert 4500 ms に設定されており、特に expert は hard stop 4.0秒を踏み抜く設定になっている。今回の改善で hard stop を厳格化する以上、現行値の流用は不可
- 難易度別の探索予算は **Phase 0 ベンチの p50 / p95 / max 実測値** を見て確定する。仮置きの目安は beginner 0.8秒、intermediate 1.5秒、advanced 3.0秒、expert 3.5秒だが、ベンチ結果に応じて調整する。どの難易度でも hard stop は 4.0秒以内に収める
...
- app session / guest session の確認は、Server Action と同様に既存helper `getCurrentAppUser()` (`src/lib/auth/current-user.ts`) を再利用する。Clerk/auth直叩きと混在させず、guest cookie 経路も同 helper 経由に統一する
...
実装前に Next.js 16.2.1 の Route Handler / segment config / `maxDuration` の最新仕様を確認する。現行worktreeには `node_modules/next/dist/docs` が存在しないため、ローカルdocsが取得できる環境または公式docsで確認してから `runtime` / `maxDuration` の記述を確定する。あわせて Vercel 側の `maxDuration` 実上限 (契約プランによる) も確認する。
...
### AI Route Handler 失敗時の UX

ネットワーク失敗、HTTP 5xx、`maxDuration` 超過、サーバ内部エラー、abort 以外の例外で AI が手を返せなかった場合、card-shogi は AI が止まると進行不能になるため次の方針で UX を保つ。

- **自動リトライ**: 1〜2 回まで指数バックオフ (例: 600ms → 1.5s) で再試行する。`abort` (= 待った/終局/unmount) によるキャンセルはリトライ対象外
- **連続失敗時のエラーモーダル**: 自動リトライが尽きた場合は、対局上に「AI が応答しませんでした。もう一度試す / 投了する」を提示するエラーモーダルを表示する。`isAiThinking` は同時に false に戻し、UI を進行可能状態へ復旧する
- **手動再考**: モーダルの「もう一度試す」で同一局面に対し新規 request id で再 fetch する
- リトライとモーダル表示は client helper (`use-ai-request.ts` 想定) に集約し、標準将棋とカード将棋の両フックから共通利用する
- リトライ中も `isAiThinking` は維持するが、合計待ち時間が hard stop (探索5秒 + リトライ計2回 + バックオフ) を踏み抜かないよう client-side overall timeout (例: 15 秒) で打ち切る
- リトライ・モーダル発火・手動再考の各イベントは stats logging に残し、Phase 5 検証で発火頻度を確認する

## 実装ステップ

### PR 段階分割方針

本Issueは規模が大きいため、PRを2本に分割して段階的にマージする。

- **PR 1 (`feature/#176` → main): Phase 0 + Phase 1 + Phase 2**
  - 経路分離 (Server Action → Route Handler) と deadline 付き探索の導入
  - 500ms 撤廃・saveMove .catch 追加・stale 応答無視 などの UX 安全網
  - benchmark で max 5 秒未満・p50 1〜3 秒 を確認できた段階で push して停止
- **PR 2 (PR 1 マージ後の派生ブランチ `feature/#176-evaluate-improve` 等): Phase 3 + Phase 4 + Phase 5**
  - 探索用合法手生成、評価関数の責務分割、blunder guard 見直し、棋力回帰 fixture
  - 棋力 fixture 全パス + benchmark 安定を確認した段階で push して停止

各 PR で Issue #109 観点の3段階レビュー (計画確定後・コミット前・PR レビュー前) を実施する。
...
- 計測用スクリプトは `scripts/bench-ai.ts` に置く
- 出力は p50 / p95 / max / nodes / depthCompleted / stoppedBy / timedOut を含む JSON とし、`bench-results/` 配下に保存する。`bench-results/` は `.gitignore` 対象にして履歴をコミットしない (CI artifact 化は別途検討)
- difficulty × phase × variant × 局面種別 (王手中 / 通常 / 終盤詰めろ) を網羅する fixture セットを `src/lib/shogi/ai/__tests__/fixtures/` に置き、Phase 4 の engine-strength.test.ts でも再利用する

### Phase 1: AI呼び出し経路の分離

- `src/app/api/ai-move/route.ts` を追加する
- `AiMoveRequest` / `AiMoveResponse` / `SearchStats` の型を整理する
- 入力検証、同一origin確認、session確認 (`getCurrentAppUser()` 再利用)、所有者確認、サイズ制限、多重request抑制を入れる
- hooks から Server Action ではなく fetch helper を呼ぶ
- 標準将棋の固定500ms待ちを撤廃する
- 標準将棋の `saveMove` はプレイヤー/AIどちらも fire-and-forget + catch に揃える
- **API 移行ポリシーは「完全置換」**: 旧 `src/app/actions/ai.ts` および旧 `calculateAiMove(state, player, difficulty, variant): Move | null` を削除し、新 API `findBestMove(state, player, difficulty, variant, ctx?: SearchContext): { move: Move | null; stats: SearchStats }` に統一する。互換ラッパーは残さない (デッドコード防止 + AGENTS.md「実装ガイドライン 10」)
- AI 失敗時 UX (リトライ + エラーモーダル) を `use-ai-request.ts` helper に集約する
...
- `pnpm run lint`
- `pnpm run typecheck`
- `pnpm run test:ci` (`test` は vitest watch のため CI 用は `test:ci`)
- `pnpm run build`
...
- `pnpm run lint`、`pnpm run typecheck`、`pnpm run test:ci`、`pnpm run build` の結果を報告できる
